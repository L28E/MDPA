function AddEllipAtomicArray(xwidth,ywidth, X0, Y0, VX0, VY0, InitDist, Temp, Type)
global C
...
L = (2*xwidth - 1) * AtomSpacing;
W = (2*ywidth - 1) * AtomSpacing;

xp(1, :) = linspace(-L/2, L/2, 2*xwidth);
yp(1, :) = linspace(-W/2, W/2, 2*ywidth);

numAtoms = 0;
for i = 1:2*xwidth
    for j = 1:2*ywidth
        if (xp(i)/(xwidth*AtomSpacing))^2 + (yp(j)/(ywidth*AtomSpacing))^2 <= 1
            numAtoms = numAtoms+1;
